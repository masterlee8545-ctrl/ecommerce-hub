#!/usr/bin/env node
/**
 * 정리 v4 — 형 진입 기준 종합 자동 적용
 *
 * 자동 처리:
 *   - 샐러드 삭제 (한국 브랜드 강세 신선식품)
 *   - 앵두 → 유어밸류 이전 (농산물 분류 오류)
 *   - 빈 자리 새 후보 공산품 위주 (농산물 비율 균형)
 *
 * 학습된 형 기준 (자동 적용):
 *   ❌ 화장품/의류/영양제/가전/명품
 *   ❌ 부피 큰 가구
 *   ❌ 식품 제조 (라면/면류/만두/떡/샐러드/핫도그 등 — 한국 브랜드 강세)
 *   ❌ 음료 (커피/탄산수/차/즙/액 완전 배제)
 *   ❌ 세제류
 *   ❌ 광범위 카테고리 단어
 *   ❌ 육가공
 *   ❌ 마스크팩
...
const BEVERAGE_PATTERN =
  /^(음료|주스|콜라|사이다|탄산수|생수|우유|두유|커피|아이스커피|아메리카노|라떼|에이드|밀크티|보이차|보리차|국화차|허브차|녹차|홍차|율무차|쌍화차|생강차|대추차|꿀차|유자차|매실차|식혜|수정과|쥬스)$|즙(?!기|석)$|^[가-힣]+액$|에이드$/;
...
const EXCLUDE_PATTERNS: Array<{ category: string; pattern: RegExp }> = [
  ...
  { category: '포괄적', pattern: /^(워터파크|물놀이|스포츠레저|축구|야구|농구|배구|등산|캠핑|레저|운동|피트니스|건강|뷰티|패션|취미|문구|놀이|여행|쇼핑)$/ },
  ...
  // 식품 제조 (한국 브랜드 강세 + 인허가) — 샐러드 추가
  { category: '식품제조/신선식품', pattern: /^(면류|곤약면|만두|즉석밥|컵라면|컵면|햇반|떡(?!볶이)|쌀과자|국|반찬|김치|장아찌|젓갈|샐러드|핫도그|라면|치킨|피자|과자|초콜릿|아이스크림|짜장|우동|국수|반조리|간편식|냉동식품|밀키트|도시락|샌드위치|샌드위치|버거|돈까스|돈가스)$/ },
...
function guessCategory(keyword: string, isAgri: boolean): string {
  ...
  if (/인테리어|시트지|타일|벽지|쿠션|액자|조명|선반/.test(keyword)) return '인테리어';
...
(async () => {
  ...
  // 1) 샐러드 삭제 (한국 브랜드 강세)
  console.log('=== 1단계: 샐러드 자동 삭제 ===');
  const deleted = await withCompanyContext(indCo.id, async (tx) => {
    return tx.delete(products).where(and(eq(products.company_id, indCo.id), eq(products.name, '샐러드'))).returning({ name: products.name, code: products.code });
  });
  for (const d of deleted) console.log(`  ✅ 삭제: ${d.code} ${d.name}`);

  // 2) 앵두 → 유어밸류 이전 (농산물)
  console.log('\n=== 2단계: 앵두 → 유어밸류 이전 ===');
  const cherries = await withCompanyContext(indCo.id, async (tx) => {
    return tx.select().from(products).where(and(eq(products.company_id, indCo.id), eq(products.name, '앵두')));
  });
  for (const p of cherries) {
    const newCode = await suggestNextProductCode(agriCo.id);
    await withCompanyContext(agriCo.id, async (tx) => {
      await tx.insert(products).values({
        company_id: agriCo.id, code: newCode, name: p.name, category: '농산물',
    ...
    console.log(`  ✅ 앵두 이전 → 유어밸류 ${newCode}`);
  }
  await withCompanyContext(indCo.id, async (tx) => {
    await tx.delete(products).where(and(eq(products.company_id, indCo.id), eq(products.name, '앵두')));
  });

  // 3) 빈 자리 새 후보 — 공산품 우선 (현재 농산물 비율 높음)
  ...
  for (const c of top) {
    ...
    await withCompanyContext(targetCo.id, async (tx) => {
      await tx.insert(products).values({
        ...
        description: `시즌 점수 ${c.score}점, ${c.seasonalityRatio.toFixed(1)}배. v4.`,
