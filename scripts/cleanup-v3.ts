...
const BEVERAGE_PATTERN =
  /^(음료|주스|콜라|사이다|탄산수|생수|우유|두유|커피|아이스커피|아메리카노|라떼|에이드|냉음료|온음료|밀크티|보이차|보리차|국화차|허브차|녹차|홍차|율무차|쌍화차|생강차|대추차|꿀차|유자차|매실차|식혜|수정과)$|즙(?!기|석)$|^[가-힣]+액$|에이드$/;

const EXCLUDE_PATTERNS: Array<{ category: string; pattern: RegExp }> = [
  { category: '화장품', pattern: /화장품|로션|크림|세럼|토너|에센스|미스트|마스카라|아이라이너|아이섀도|립스틱|블러셔|컨실러|쿠션|파운데이션|선크림|선스틱|샴푸|린스|트리트먼트|헤어팩|두피\s*샴푸|미백|메이크업|네일|매니큐어|향수|퍼퓸|코롱|디퓨저|향초|마스크\s*\/?\s*팩|마스크팩|시트마스크/ },
  { category: '의류', pattern: /원피스|티셔츠|셔츠|블라우스|니트|가디건|스웨터|자켓|점퍼|패딩|코트|청바지|진(?!액|짜)|바지|레깅스|치마|스커트|수영복|비키니|레쉬가드|등산복|러닝화|운동화|스니커즈|로퍼|구두|샌들|슬리퍼|부츠|양말|스타킹|속옷|브라|팬티/ },
  ...
  { category: '포괄적', pattern: /^(워터파크|물놀이|스포츠레저|축구|야구|농구|배구|등산|캠핑|레저|운동|피트니스|건강|뷰티|패션|취미|문구|놀이|여행)$/ },
  ...
  { category: '식품제조', pattern: /^(면류|곤약면|만두|즉석밥|컵라면|컵면|햇반|떡|쌀과자|국|반찬|김치|장아찌|젓갈)$/ },
  { category: '세제류', pattern: /섬유유연제|주방세제|빨래세제|세탁세제|얼룩|표백제|살균|소독제(?!.*수)/ },
  { category: '한국 대형 브랜드', pattern: /^(라면|핫도그|샐러드|치킨|피자|과자|초콜릿|아이스크림|짜장|우동|국수)$/ },
  // v3 신규
...
(async () => {
  ...
  // 1) 음료 키워드 삭제
  console.log('=== 1단계: 음료 삭제 ===');
  const TO_DELETE = ['아이스커피', '탄산수', '매실액'];
  for (const co of [agriCo, indCo]) {
    const deleted = await withCompanyContext(co.id, async (tx) => {
      return tx.delete(products).where(and(eq(products.company_id, co.id), inArray(products.name, TO_DELETE))).returning({ name: products.name });
    });
    if (deleted.length > 0) console.log(`  [${co.name}] 삭제: ${deleted.map((d) => d.name).join(', ')}`);
  }

  // 2) 망고 → 유어밸류 이전
  console.log('\n=== 2단계: 망고 → 유어밸류 이전 ===');
  const mangoes = await withCompanyContext(indCo.id, async (tx) => {
    return tx.select().from(products).where(and(eq(products.company_id, indCo.id), eq(products.name, '망고')));
  });
  for (const p of mangoes) {
    const newCode = await suggestNextProductCode(agriCo.id);
    await withCompanyContext(agriCo.id, async (tx) => {
      await tx.insert(products).values({
        company_id: agriCo.id, code: newCode, name: p.name, category: '농산물(수입과일)',
        status: 'research', description: p.description, supply_type: 'domestic_vendor',
        season_peak_month: p.season_peak_month, season_prep_month: p.season_prep_month,
        seasonality_ratio: p.seasonality_ratio, season_score: p.season_score,
      });
    });
    console.log(`  ✅ 망고 이전 → 유어밸류 ${newCode}`);
  }
  await withCompanyContext(indCo.id, async (tx) => {
    await tx.delete(products).where(and(eq(products.company_id, indCo.id), eq(products.name, '망고')));
  });

  // 3) 빈 자리 채우기
  const existingProducts = await db.select({ name: products.name }).from(products).where(eq(products.status, 'research'));
  const existingNames = new Set(existingProducts.map((p) => p.name));
  const needed = Math.max(0, TARGET_TOTAL - existingNames.size);
  console.log(`\n현재 ${existingNames.size}개 / 목표 ${TARGET_TOTAL}개 → 추가 ${needed}개\n`);
  ...
  candidates.sort((a, b) => a.score !== b.score ? b.score - a.score : b.seasonalityRatio - a.seasonalityRatio);
  ...
  for (const c of top) {
    ...
    await withCompanyContext(targetCo.id, async (tx) => {
      await tx.insert(products).values({
        ...
        description: `시즌 점수 ${c.score}점, ${c.seasonalityRatio.toFixed(1)}배. 자동 발굴 v3.`,
